	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.jiaolianName" type="text" placeholder="教练名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('jiaolian','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('jiaolian','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='jiaolianTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in jiaolianTypesList" :key="index" :class="jiaolianTypes===item.codeIndex?'tabActive':''"
                                  @tap="jiaolianTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <block v-for="(product,index) in list" :key="index">
            <view v-if="index%6==0" @tap="onDetailTap(product)" class="list-item"
                ...
                <image
                        ...
                        mode="aspectFill" :src="product.jiaolianPhoto?baseUrl+product.jiaolianPhoto.split(',')[0]:''">
                </image>
                <view class="list-item-body" style="display: flex;flex-wrap: wrap;">
                    <view
                            ...
                            class="title">{{product.jiaolianName}}
                    </view>
                    <view v-if='product.jiaolianNewMoney'
                          ...
                          class="title">{{product.jiaolianNewMoney}}
                    </view>
                    <view v-else-if='product.jiaolianValue'
                          :style='{"padding":"0 24rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'
                          class="title">{{product.jiaolianValue}}
                    </view>
                </view>
                <view style="display: flex;justify-content: space-between;">
                    <text v-if="isAuth('jiaolian','修改')" class="cuIcon-edit"
                          @click.stop="onUpdateTap(product.id)">修改
                    </text>
                    <text v-if="isAuth('jiaolian','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 教练类型 字段 字典表查询方法
            async jiaolianTypesSelect(){
                let jiaolianTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'jiaolian_types',
                });
                jiaolianTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.jiaolianTypesList = jiaolianTypes.data.list
            },
            //当前表的 教练类型字段类别搜索
			jiaolianTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.jiaolianTypes != 0) {
                    params.jiaolianTypes = this.jiaolianTypes
                }
                let _this = this
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.jiaolianShenheTime=shijian;
                            await _this.$api.update(`jiaolian`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.jiaolianName) {
                    searchForm['jiaolianName'] = this.searchForm.jiaolianName
                }
