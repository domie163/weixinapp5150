			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.jiaolianName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
            ...
            <!-- 留言 -->
				...
				<view class="bottom-content cu-bar bg-white tabbar border shop">
					<view style="width: 100%;margin: 0 auto;display: flex;align-items: center;justify-content: space-around;">
                      ...
                      <view @tap="baoming" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.jiaolianId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getjiaolianCollection()
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
                        jiaolianId: this.jiaolianId,
                        yonghuId: this.user.yonghuId,
						jiaolianLiuyanPingfenNumber: this.scoreData,
                        jiaolianLiuyanText: this.commentData,
                    }
                    await this.$api.save(`jiaolianLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('jiaolian', this.jiaolianId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.jiaolianPhoto ? this.detail.jiaolianPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getjiaolianCollection() {
					let params = {
						page: 1,
						limit: 1,
						jiaolianId: this.jiaolianId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`jiaolianCollection`, params);
					if (res.data.list.length == 1) {
						let jiaolianCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('jiaolianCollection', JSON.stringify([jiaolianCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getjiaolianCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('jiaolianCollection', {
									yonghuId: _this.user.id,
									jiaolianId: _this.detail.id,
									jiaolianCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('jiaolianLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						jiaolianId: this.jiaolianId
					});
