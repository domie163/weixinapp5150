				...
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app">
								{{detail.jiaolianCollectionValue}}
							</view>
						</view>
					</view>
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.jiaolianCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('jiaolianCollection', this.jiaolianCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.jiaolianCollectionPhoto ? this.detail.jiaolianCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('jiaolianCollectionYuyue', {
									yonghuId: _this.user.id,
									jiaolianCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/jiaolianCollectionYuyue/list');
							}
						}
					});
				},
