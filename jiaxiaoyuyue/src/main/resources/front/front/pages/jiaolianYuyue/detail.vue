	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.jiaolianYuyueId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('jiaolianYuyue', this.jiaolianYuyueId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.jiaolianYuyuePhoto ? this.detail.jiaolianYuyuePhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('jiaolianYuyueYuyue', {
									yonghuId: _this.user.id,
									jiaolianYuyueId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/jiaolianYuyueYuyue/list');
							}
						}
					});
				},
