	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.kaoshixName" type="text" placeholder="通知标题"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('kaoshix','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('kaoshix','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='kaoshixTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in kaoshixTypesList" :key="index" :class="kaoshixTypes===item.codeIndex?'tabActive':''"
                                  @tap="kaoshixTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <block v-for="(product,index) in list" :key="index">
            <view v-if="index%6==0" @tap="onDetailTap(product)" class="list-item"
                ...
                <image
                        ...
                        mode="aspectFill" :src="product.kaoshixPhoto?baseUrl+product.kaoshixPhoto.split(',')[0]:''">
                </image>
                <view class="list-item-body" style="display: flex;flex-wrap: wrap;">
                    <view
                            ...
                            class="title">{{product.kaoshixName}}
                    </view>
                    <view v-if='product.kaoshixNewMoney'
                          ...
                          class="title">{{product.kaoshixNewMoney}}
                    </view>
                    <view v-else-if='product.kaoshixValue'
                          :style='{"padding":"0 24rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'
                          class="title">{{product.kaoshixValue}}
                    </view>
                </view>
                <view style="display: flex;justify-content: space-between;">
                    <text v-if="isAuth('kaoshix','修改')" class="cuIcon-edit"
                          @click.stop="onUpdateTap(product.id)">修改
                    </text>
                    <text v-if="isAuth('kaoshix','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 考试类型 字段 字典表查询方法
            async kaoshixTypesSelect(){
                let kaoshixTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'kaoshix_types',
                });
                kaoshixTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.kaoshixTypesList = kaoshixTypes.data.list
            },
            //当前表的 考试类型字段类别搜索
			kaoshixTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.kaoshixTypes != 0) {
                    params.kaoshixTypes = this.kaoshixTypes
                }
                let _this = this
                let res = await this.$api.list(`kaoshix`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('kaoshix', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.kaoshixShenheTime=shijian;
                            await _this.$api.update(`kaoshix`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.kaoshixName) {
                    searchForm['kaoshixName'] = this.searchForm.kaoshixName
                }

                let res = await this.$api.list(`kaoshix`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.kaoshixContent!=null){
                        item.kaoshixContent = item.kaoshixContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
