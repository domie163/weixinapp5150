	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.jiaolianLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('jiaolianLiuyan', this.jiaolianLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.jiaolianLiuyanPhoto ? this.detail.jiaolianLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('jiaolianLiuyanYuyue', {
									yonghuId: _this.user.id,
									jiaolianLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/jiaolianLiuyanYuyue/list');
							}
						}
					});
				},
