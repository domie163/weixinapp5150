	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.kaoshixYuyueId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('kaoshixYuyue', this.kaoshixYuyueId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.kaoshixYuyuePhoto ? this.detail.kaoshixYuyuePhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('kaoshixYuyueYuyue', {
									yonghuId: _this.user.id,
									kaoshixYuyueId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/kaoshixYuyueYuyue/list');
							}
						}
					});
				},
